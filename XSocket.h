...
#define DEF_XSOCKEVENT_SOCKETMISMATCH \
  -121  // 소켓 이벤트와 소켓이 서로 다르다. (이런 경우가?)
...
#define DEF_XSOCKEVENT_SOCKETERROR \
  -129  // 소켓에 에러가 발생했다. 이경우 클래스를 삭제하여야 한다.
#define DEF_XSOCKEVENT_CRITICALERROR \
  -130  // 치명적인 에러로 프로그램 전체가 멈춰야 한다.
#define DEF_XSOCKEVENT_NOTINITIALIZED \
  -131  // 클래스가 초기화 되지 않은 상태로 사용됐다.
#define DEF_XSOCKEVENT_MSGSIZETOOLARGE \
  -132  // 보내고자 하는 메시지의 사이즈가 너무 크다.
#define DEF_XSOCKEVENT_CONFIRMCODENOTMATCH \
  -133  // 확인코드가 일치하지 않는다. 삭제되어야 한다.
#define DEF_XSOCKEVENT_QUENEFULL -134  // 블록 큐의 공간이 없다.
#define DEF_XSOCKEVENT_UNSENTDATASENDBLOCK \
  -135  // 큐에 있는 데이터를 보내다가 또 블록이 걸렸다.
#define DEF_XSOCKEVENT_UNSENTDATASENDCOMPLETE \
  -136  // 큐에 있는 모든 데이터를 보냈다.

#define DEF_XSOCKBLOCKLIMIT \
  300  // 이 클래스 소켓은 ?번이상 블록되면 더이상 데이터를 저장할 수 없다.
