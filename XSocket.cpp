...
int XSocket::iOnSocketEvent(WPARAM wParam, LPARAM lParam) {
  ...
  switch (WSAEvent) {
    ...
    case FD_CLOSE:
      m_cType =
          DEF_XSOCK_SHUTDOWNEDSOCK;  // 소켓이 닫혔으므로 이 소켓은 사용 불가능.
...
BOOL XSocket::bConnect(char *pAddr, int iPort, unsigned int uiMsg) {
  ...
  // 소켓 옵션을 조정한다.
  dwOpt = 8192 * 5;
  setsockopt(m_Sock, SOL_SOCKET, SO_RCVBUF, (const char FAR *)&dwOpt,
             sizeof(dwOpt));
  setsockopt(m_Sock, SOL_SOCKET, SO_SNDBUF, (const char FAR *)&dwOpt,
             sizeof(dwOpt));
...
int XSocket::_iOnRead() {
  ...
  // 메시지를 모두 읽었다. 이 메시지를 받으면 클래스 버퍼의 내용을 읽어 즉각
  // 처리해야 한다.
  return DEF_XSOCKEVENT_READCOMPLETE;
}
  ...
  while (iOutLen < iSize) {
    ...
    if (iRet == SOCKET_ERROR) {
      ...
      if (WSAErr != WSAEWOULDBLOCK) {
        ...
        // 블럭상태이면 더이상 보낼 수 없으므로 남아있는 데이터를 리스트에
        // 등록하고 리턴
        if (bSaveFlag == TRUE) {
...
// 이 함수는 _SendUnsentData에서만 사용한다. 블록이 아닌 에러시에는 음수값을,
// 블럭상태를 포함한 전송상태에서는 보낸 만큼의 값을 반환.
  ...
  while (iOutLen < iSize) {
    ...
    if (iRet == SOCKET_ERROR) {
      ...
      if (WSAErr != WSAEWOULDBLOCK) {
        ...
        // 블럭상태이면 더이상 보낼 수 없으므로 지금까지 보낸 데이터 사이즈만을
        // 반환
        return iOutLen;
      }
    } else
...
int XSocket::_iSendUnsentData() {
  ...
  while (m_pUnsentDataList[m_sHead] != NULL) {
    ...
    if (iRet == m_iUnsentDataSize[m_sHead]) {
      ...
      // 데이터를 다 보내지 못하고 또 블록 상태가 발생했다. 보내지 못한 데이터만
      // 남겨놓는다.
...
BOOL XSocket::bAccept(class XSocket *pXSock, unsigned int uiMsg) {
  ...
  // 클라이언트의 접속을 받는다 .
  AcceptedSock =
      accept(m_Sock, (struct sockaddr FAR *)&Addr, (int FAR *)&iLength);
  ...
  // 소켓 옵션을 조정한다.
  dwOpt = 8192 * 5;
  setsockopt(pXSock->m_Sock, SOL_SOCKET, SO_RCVBUF, (const char FAR *)&dwOpt,
             sizeof(dwOpt));
  setsockopt(pXSock->m_Sock, SOL_SOCKET, SO_SNDBUF, (const char FAR *)&dwOpt,
             sizeof(dwOpt));
