...
unsigned __stdcall ThreadProc(void *ch) {
  ...
  while (G_bThreadFlag == TRUE) {
    ...
    if (iCurrentUserTime > 60 * 1) {
      ...
      for (i = 1; i < 500; i++) {
        if ((G_pGateCore->m_pClientList[i] != NULL) &&
            (G_pGateCore->m_pClientList[i]->m_bIsGameServer == TRUE)) {
          wsprintf(cTemp, "(%s) Total user(%d)",
                   G_pGateCore->m_pClientList[i]->m_cName,
                   G_pGateCore->m_pClientList[i]->m_iTotalClients);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  switch (message) {
    ...
    case WM_CLOSE:
      if (MessageBox(NULL, "Quit gate server?", "GATE-SERVER",
...
void Initialize() {
  ...
  G_pListenSock->bListen(G_pGateCore->m_cGateServerAddr,
                         G_pGateCore->m_iGateServerPort, WM_USER_ACCEPT);
...
void OnPaint() {
  ...
  TextOut(hdc, 10, 10, "[F1 + F4]  Shut down all game servers",
          strlen("F1 + F4 : Shut down all game servers"));
  TextOut(hdc, 10, 25, "[F2] Shows not activated game server list",
          strlen("[F2] Shows not activated game server list"));
  TextOut(hdc, 10, 40, "[F3] Shows activated game server list",
          strlen("[F3] Shows activated game server list"));

  TextOut(hdc, 0, 620,
          "____________________________________________________________________"
          "____________",
          strlen("_____________________________________________________________"
                 "___________________"));
  if (G_pGateCore != NULL) {
    for (i = 0; i < DEF_MAXMONITORS; i++) {
      ...
      switch (G_pGateCore->m_cMonitorStatus[i]) {
        default:
          TextOut(hdc, 20 + i * 30, 655, "?", 1);
          break;
          break;
        case 1:
          TextOut(hdc, 20 + i * 30, 655, "X", 1);
          break;
        case 2:
          TextOut(hdc, 20 + i * 30, 655, "O", 1);
          break;
      }
    }
  }
...
void OnAccept() { G_pGateCore->bAccept(G_pListenSock); }
...
void PutGameServerList(char *cStr) {
  ...
  ZeroMemory(cBuffer, sizeof(cBuffer));
  wsprintf(cBuffer, "GameServerLog\\GameServerLog%04d%02d%02d.log",
