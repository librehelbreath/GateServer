...
PartyManager::PartyManager(class CGateCore *pGateCore) {
  ...
  for (i = 0; i < DEF_MAXPARTY; i++) {
    ...
    ZeroMemory(m_stMemberNameList[i].m_cName,
               sizeof(m_stMemberNameList[i].m_cName));
  }
...
int PartyManager::iCreateNewParty(int iClientH, char *pMasterName) {
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if ((m_stMemberNameList[i].m_iPartyID != NULL) &&
        (strcmp(m_stMemberNameList[i].m_cName, pMasterName) == 0))
      return 0;
...
BOOL PartyManager::bAddMember(int iClientH, int iPartyID, char *pMemberName) {
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if ((m_stMemberNameList[i].m_iPartyID != NULL) &&
        (strcmp(m_stMemberNameList[i].m_cName, pMemberName) == 0)) {
      m_stMemberNameList[i].m_iPartyID = NULL;
      m_stMemberNameList[i].m_iIndex = NULL;
      m_stMemberNameList[i].m_dwServerChangeTime = NULL;
      ZeroMemory(m_stMemberNameList[i].m_cName,
      ...
      m_iMemberNumList[iPartyID]--;
      if (m_iMemberNumList[iPartyID] <= 1)
        bDeleteParty(iPartyID);  // 파티원이 1명만 남으면 파티 제거
    }
  //		return FALSE;
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if (m_stMemberNameList[i].m_iPartyID == NULL) {
      ...
      // testcode
      // wsprintf(G_cTxt, "Add Member: PartyID:%d Name:%s", iPartyID,
      // pMemberName);  PutLogList(G_cTxt);
      return TRUE;
    }
...
BOOL PartyManager::bRemoveMember(int iPartyID, char *pMemberName) {
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if ((m_stMemberNameList[i].m_iPartyID == iPartyID) &&
      ...
      // testcode
      // wsprintf(G_cTxt, "Remove Member: PartyID:%d Name:%s", iPartyID,
      // pMemberName);  PutLogList(G_cTxt);
      return TRUE;
    }
...
BOOL PartyManager::bCheckPartyMember(int iClientH, int iGSCH, int iPartyID,
                                     char *pName) {
  int i, iRet;
  char *cp, cData[120];
  DWORD *dwp;
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if ((m_stMemberNameList[i].m_iPartyID == iPartyID) &&
        (strcmp(m_stMemberNameList[i].m_cName, pName) == 0)) {
      // 서버 이동 타임 클리어
      m_stMemberNameList[i].m_dwServerChangeTime = NULL;
      return TRUE;
    }
...
BOOL PartyManager::bGetPartyInfo(int iClientH, int iGSCH, char *pName,
                                 int iPartyID) {
  int i, iRet, iTotal;
  char *cp, cData[1024];
  DWORD *dwp;
  ...
  *wpTotal = iTotal;
  iRet = m_pGateCore->m_pClientList[iClientH]->m_pXSock->iSendMsg(
      cData, 20 + iTotal * 11 + 1);
...
void PartyManager::GameServerDown(int iClientH) {
  ...
  for (i = 0; i < DEF_MAXPARTY; i++)
    if (m_stMemberNameList[i].m_iIndex == iClientH) {
      // testcode
      // wsprintf(G_cTxt, "Removing Party member(%s) by Server down",
      // m_stMemberNameList[i].m_cName);  PutLogList(G_cTxt);
...
void PartyManager::CheckMemberActivity() {
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if ((m_stMemberNameList[i].m_dwServerChangeTime != NULL) &&
        ((dwTime - m_stMemberNameList[i].m_dwServerChangeTime) > 1000 * 20)) {
      // 시간 초과.
      ZeroMemory(cData, sizeof(cData));
      cp = (char *)cData;
      dwp = (DWORD *)cp;
      ...
      *wp = (WORD)m_stMemberNameList[i].m_iPartyID;
      ...
      bRemoveMember(m_stMemberNameList[i].m_iPartyID,
                    m_stMemberNameList[i].m_cName);
    }
}
